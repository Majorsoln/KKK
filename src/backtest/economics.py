...
class Economics:
    ...
    def edge_over_live(self) -> float:
        """Uwiano WENYE MAMLAKA — ndio unaopita au kukataliwa (R20)."""
    ...
    def render(self) -> str:
        ...
        return (
            ...
            f"edge/live {self.edge_over_live:>5.2f}× (dai {KIZIDISHI:.0f}×) · "
            f"sens {self.cost_sensitivity:>4.2f}×  {alama}"
        )
...
def measure(result) -> Economics:
    ...
    # hesabu mpya.
    edge = sum(t.path.gross_pips + t.path.spread_pips for t in trades) / n
    research = sum(
